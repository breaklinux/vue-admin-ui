...
<script>
import {getLdapUserList, postLdapUserList, deletedapUserList } from '@/api/user'
...
export default {
  ...
  methods: {
    ...
    getList() {
      getLdapUserList(this.listQuery).then(response => {
    ...
    postList() {
      postLdapUserList(this.temp).then(response => {
    ...
    delHost(row) {
      ...
      _this51.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {
        ...
        deletedapUserList(row).then(response =>{
